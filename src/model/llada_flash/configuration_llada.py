...
# The stock ``src.model.llada`` package registers the same "llada" model type.
# Importing both packages in one process must not raise, and whichever config
# class wins the Auto* registry is irrelevant here: the flash model is always
# instantiated directly as LLaDAFlashModelLM, never through AutoModel.
try:
